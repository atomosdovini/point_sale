...
class Basket
  ...
  def initialize(file)
    ...
    # converts @list to a new array @output, remove "at"
    # Only quanity&name ("1 book", "2 CDs", 
    @output = []
    @list.each do |i|
      new_line = i.split(' at ')
      @output << new_line
...
class TestBasket < Minitest::Test
  ...
  def test_textfile_parse
    assert_equal ['1 book at 12.49', '1 music CD at 14.99', '1 chocolate bar at 0.85'], @basket.list
  end
...
list1 = Basket.new('input1.txt')
puts list1.print_receipt

puts '*' * 50
list2 = Basket.new('input2.txt')
puts list2.print_receipt

puts "*" * 50
list3 = Basket.new('input3.txt')
puts list3.print_receipt
